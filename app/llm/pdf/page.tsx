...
const FileUploadCard: React.FC = () => {
  ...
  const [uploadedFilesx, setUploadedFilesx] = useState<File[]>([]);


  const handleFileDropx = (e: React.DragEvent<HTMLDivElement>) => {
    e.preventDefault();
  ...
  const handleFileInputx = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (validFiles.length > 0) {
      setUploadedFilesx((prevFiles) => [...prevFiles, ...validFiles]);
    } else {
      alert("Unsupported file type. Please upload a PDF, DOC, TXT, or CSV file.");
    }
  };

  const removeFilex = (index: number) => {
    setUploadedFilesx((prevFiles) => prevFiles.filter((_, i) => i !== index));
  };
  const handlePostDatax = async (): Promise<void> => {
...
<div className="h-32 overflow-y-scroll " >{uploadedFiles.length > 0 && (
  ...
  onDrop={(e) => {
    e.preventDefault();
    e.stopPropagation(); // Stops propagation beyond this element
    handleFileDropx(e); // Process files
  }}
  onDragOver={(e) => {
    e.preventDefault();
    e.stopPropagation(); // Stops propagation beyond this element
